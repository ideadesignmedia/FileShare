...
export async function storeTempShareFiles(shareToken: string, files: File[], onProgress?: (fileIndex: number, percent: number) => void): Promise<string[]> {
  ...
  for (let idx = 0; idx < files.length; idx++) {
    ...
    const relativePath = (file as any).webkitRelativePath || file.name
    const folderRoot = relativePath.includes('/') ? relativePath.split('/')[0] : undefined
    await saveFileMetadata(fileId, file.name, file.type, file.size, '', Date.now(), true, { temp: true as any, shareToken, relativePath, folderRoot })
  }
  return fileIds
}
